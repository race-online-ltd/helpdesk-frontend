...
export const ClientVsCustomerFilterComponent = ({
  closeOffcanvas,
  setIsLoadingTicketCountClientVsCustomer,
  setTicketCountClientVsCustomer,
  ...
  const fetchTicketCountClientVsCustomer = (values) => {
    getTicketCountByClientCustomer(values).then((response) => {
      setTicketCountClientVsCustomer(response.data);
      setDisplayClientVsCustomerBusinessEntityName(
        response.data?.dayWise?.[0]?.company_name
      );
    });
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      Promise.all([fetchTicketCountClientVsCustomer(values)])
        .catch(errorMessage)
        .finally(() => {
          closeOffcanvas();
          const range =
          ...
          setPrintDateRangeForClientVsCustomer(range);
