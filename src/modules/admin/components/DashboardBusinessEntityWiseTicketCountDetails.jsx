...
import {
  getTeamVsBusinessTicketCountDetailsByBusinessEntityId,
...
export const DashboardBusinessEntityWiseTicketCountDetails = ({
  closeOffcanvas,
  ...
  useEffect(() => {
    ...
    getTeamVsBusinessTicketCountDetailsByBusinessEntityId(data)
      .then((response) => {
  ...
  const agentColumns = [
    ...
    {
      name: "Assigned Agent",
      selector: (row) => row.assigned_agent_username,
      cell: (row) => (
        <div style={{ whiteSpace: "wrap" }}>
          <i className='bi bi-people-fill me-2' style={{ color: "gray" }}></i>
          <span>{row.assigned_agent_username}</span>
        </div>
      ),
    },
